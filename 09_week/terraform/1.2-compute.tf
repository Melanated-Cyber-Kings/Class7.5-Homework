...
# Instance Template
resource "google_compute_instance_template" "basic_gui" {
  ...
  network_interface {
    network    = google_compute_network.top.self_link
...
resource "google_compute_region_instance_group_manager" "webserver" {
  ...
  version {
    instance_template = google_compute_instance_template.basic_gui.self_link
  }
